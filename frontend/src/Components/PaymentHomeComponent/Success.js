...
export default class Success extends Component {
  ...
  componentDidMount() {
    ...
    this.setState({ amount: this.props.match.params.price });
  }
  ...
  onSubmit = (e) => {
    ...
    if (this.state.take == "card") {
      ...
      axios.post(`http://localhost:5002/Payments/credit`, card).then((res) => {
        let x = res.data;
        x = x.toString();
      ...
      axios
        .post(`http://localhost:5002/Payments/mobile`, mobile)
        .then((res) => {
          let x = res.data;
          x = x.toString();
          if (
            x ==
            "Your Payment is Successfully done through your service provider"
          ) {
            alert("Payment Success");
            this.setState({ redirect: true });
          } else {
